...
import { Form, FormControl, Table, Pagination, Spinner } from 'react-bootstrap'
    ...
    <div className='crypto-list' >
      <div className='market-cap'>Cryptocurrency Prices by Market Cap </div>
       <Form style={{width:"80%", height:"50px"}} className="d-flex">
        ...
        <div className="table">
      {loading ? <div className="crypto-list"><Spinner animation="border" variant="warning" /></div>  :
      <Table style={{ width: "80%" }} striped bordered hover variant="dark">
        ...
        <tbody>
          ...
          .map((coin, index) => ( 
            <tr onClick={()=> navigate(`/coins/${coin?.id}`)} key={index}>
                <td>
                ...
                <td><p className='td-el'>{symbol}{numberWithCommas(coin?.current_price.toFixed(2))}</p></td>
